...
uint64_t Exp::cast_value(reg_t t, uint64_t v)
{
...
uint32_t Exp::reg_to_bits(const reg_t &reg)
{
    switch (reg)
    {
    case REG_1:
...
void Exp::destroy(Exp *expr)
{
    ...
    case BINOP:
        ...
        BinOp::destroy((BinOp *)expr);
        break;

    case UNOP:

        UnOp::destroy((UnOp *)expr);
        break;

    case CONSTANT:

        Constant::destroy((Constant *)expr);
        break;

    case MEM:

        Mem::destroy((Mem *)expr);
        break;

    case TEMP:

        Temp::destroy((Temp *)expr);
        break;

    case PHI:

        Phi::destroy((Phi *)expr);
        break;

    case UNKNOWN:

        Unknown::destroy((Unknown *)expr);
        break;

    case CAST:

        Cast::destroy((Cast *)expr);
        break;

    case NAME:

        Name::destroy((Name *)expr);
        break;

    case LET:

        Let::destroy((Let *)expr);
        break;
...
string Exp::string_type(const reg_t &typ)
{
...
BinOp::BinOp(binop_type_t t, Exp *l, Exp *r) : Exp(BINOP), lhs(l), rhs(r), binop_type(t)
{

}

BinOp::BinOp(const BinOp &copy) : Exp(BINOP), binop_type(copy.binop_type)
{
    lhs = copy.lhs->clone();
    rhs = copy.rhs->clone();
}
...
UnOp::UnOp(const UnOp &copy) : Exp(UNOP), unop_type(copy.unop_type)
{
    exp = copy.exp->clone();
}

UnOp::UnOp(unop_type_t typ, Exp *e) : Exp(UNOP), unop_type(typ), exp(e)
{
...
Mem::Mem(Exp *a, reg_t t) : Exp(MEM), addr(a), typ(t)
{

}

Mem::Mem(const Mem &copy) : Exp(MEM)
{
...
Constant::Constant(reg_t t, const_val_t v) : Exp(CONSTANT), typ(t), val(v)
{

}

Constant::Constant(const Constant &other) : Exp(CONSTANT), typ(other.typ), val(other.val)
{
...
Constant Constant::t = Constant(REG_1, (const_val_t)1);
Constant Constant::f = Constant(REG_1, (const_val_t)0);


// ------------------------------------------------------------
// Class Phi functions
// ------------------------------------------------------------
Phi::Phi(const Phi &copy) : Exp(PHI)
{
    for (vector<Temp *>::const_iterator it = copy.vars.begin(); it != copy.vars.end(); it++)
    {
        Temp *t = *it;
...
Phi::Phi(string orig_name, vector<Temp *> v) : Exp(PHI), vars(v), phi_name(orig_name)
{
...
string Phi::tostring() const
{
    string ret = "PHI(";
    string comma = " ";

    for (vector<Temp *>::const_iterator it = vars.begin(); it != vars.end(); it++)
    {
        ret += comma;
        ret += (*it)->tostring();
        comma = ",";
...
Temp::Temp(reg_t t, string n) : Exp(TEMP), typ(t), name(n)
{ 

}

Temp::Temp(const Temp &other) : Exp(TEMP), typ(other.typ), name(other.name)
{
...
Unknown::Unknown(const Unknown &other) : Exp(UNKNOWN), str(other.str), typ(other.typ)
{ 

}

Unknown *Unknown::clone() const
{
...
Cast::Cast(Exp *e, reg_t w, cast_t t) : Exp(CAST), exp(e), typ(w), cast_type(t)
{ 

}

Cast::Cast(const Cast &other) : Exp(CAST), typ(other.typ), cast_type(other.cast_type)
{
    exp = other.exp->clone();
}
...
BinOp *_ex_xor(Exp *arg1, Exp *arg2, Exp *arg3, Exp *arg4, Exp *arg5, Exp *arg6, Exp *arg7, Exp *arg8)
{
